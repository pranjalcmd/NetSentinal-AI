    ...
    <div className="space-y-8 font-mono text-[#C1C9D6]">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-[#1E293B]/60 pb-4 gap-4">
        <div>
          <div className="flex items-center gap-2 text-[0.65rem] tracking-[0.2em] uppercase text-[#7C8798]">
            ...
            <span className="text-[#F43F5E] lowercase font-sans">incidents grouped by root cause</span>
          </div>
      ...
      <div className="space-y-4">
        {incidents.map((incident) => {
          const sev = incident.riskScore >= 80 ? 'critical' : incident.riskScore >= 60 ? 'high' : incident.riskScore >= 40 ? 'medium' : 'low';
          const sevColor = sev === 'critical' ? '#F43F5E' : sev === 'high' ? '#F59E0B' : sev === 'medium' ? '#E8C93A' : '#4B7BE5';
          return (
          <div key={incident.id} className="border border-[#1E293B]/60 bg-[#090d16] p-5 space-y-4">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 border-b border-[#1E293B]/60 pb-3">
              <div className="flex items-center gap-3">
                <span className="px-2 py-0.5 text-[0.65rem] font-bold uppercase tracking-wider bg-[#F43F5E]/10 border border-[#F43F5E]/40" style={{ color: sevColor, borderColor: `${sevColor}66` }}>
                  {sev}
                </span>
              ...
              <div className="flex items-center gap-4 text-[0.7rem]">
                <span className="text-[#7C8798]">Risk Score: <strong style={{ color: sevColor }}>{incident.riskScore}/100</strong></span>
            ...
            {incident.aiSummary && (
              <div className="border border-[#3DD9C4]/30 bg-[#3DD9C4]/5 p-3 text-[0.7rem] space-y-1">
                <span className="text-[#3DD9C4] uppercase text-[0.6rem] tracking-wider block font-bold">AI Summary</span>
